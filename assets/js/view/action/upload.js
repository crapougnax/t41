...
window.t41.view.action.upload = function(element, uri) {
	...
	this.init = function() {
		...
		var action = this._default.action;
		...
		this.qq = new qq.FileUploader({
			...
			onProgress: jQuery.proxy(this, 'onProgress'),
			onCancel: jQuery.proxy(this, 'onCancel'),
			messages: {
				typeError: "{file} n'est pas un type de fichier autorisé ({extensions}).",
				sizeError: "{file} est trop lourd, le poids maximum est {sizeLimit}.",
				minSizeError: "{file} est trop léger, le poids minimum est {minSizeLimit}.",
				emptyError: "{file} est un fichier vide.",
				noFilesError: "Pas de fichier à envoyer.",
				onLeave: "Des fichiers sont en cours d'envoi, si vous quittez la page maintenant ils seront perdus."
	        },
	        dragText: 'Déposez les fichiers ici pour les envoyer',
	        extraDropzones : [],
	        uploadButtonText: 'Choisir un fichier',
	        cancelButtonText: 'Annuler',
	        failUploadText: 'Échec de l\'envoi',
	        extraMessages: {
	            formatProgress: "{percent}% sur {total_size}",
	            tooManyFilesError: "Vous ne devez déposer qu'un seul fichier à la fois"
	        },
	...
	this.onComplete = function(id, fileName, responseJSON) {
		if (this.getLastevent()=='error' || this.getLastevent()=='cancel') {
			...
			this.setFilename(responseJSON.hash + '|' + responseJSON.filename);
	 ...
	 * @param bool boolean whether the checkbox should be checked or not
	 */
	 ...
	 * @param bool boolean turn warning on or off
	 */
	this.warning = function(bool) {
		if (bool==true) {
			this.field.addClass('warning', 500);
		} else {
