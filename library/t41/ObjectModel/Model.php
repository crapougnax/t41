...
abstract class ObjectModel extends ObjectModelAbstract {
	...
	public function __construct($val = null, array $params = null)
	{
		...
		if ($val instanceof \t41\Data\Object) {
			...
			require_once 't41/Data.php';
			$this->_dataObject = t41_Data::factory(get_class($this));
			...
			if (! is_null($val)) {
				...
				if (! $val instanceof t41_Object_Uri) {
					
					$val = new t41_Object_Uri($val);
	 ...
	 */
	public function setData(array $data, t41_Backend_Mapper $mapper = null)
	{
		...
		switch ($method_begin) {
			...
			case 'set':
				...
				if ($res === false) {
					...
					throw new t41_Object_Exception('OBJECT_UNKNOWN_PROPERTY', $method_end);
				}
			...
			case 'get':
				if (($property = $this->_dataObject->getProperty($method_end)) !== false) {
					...
					//Zend_Debug::dump($this->_dataObject); die;
					throw new t41_Object_Exception('OBJECT_UNKNOWN_PROPERTY', $method_end);
				}
				break;
			
			default:
				throw new t41_Object_Exception(array("UNKNOWN_METHOD", $m)); 
				break;
		}
	...
	public function setRules(array $rules = null)
	{
		if (is_null($rules)) {
			...
			$rules = t41_Object::getRules(get_class($this));
		}
	 ...
	 */
	public function read(t41_Backend_Adapter_Interface $backend = null)
	{
		return t41_Backend::read($this->_dataObject, $backend);
	}
	 ...
	 * @return boolean
	 */
	public function save(t41_Backend_Adapter_Interface $backend = null)
	{
		return t41_Backend::save($this->_dataObject, $backend);
	}
	
	
	public function find(array $conditions = null, array $sortings = null, $offset = 0, $batch = 10)
	{
		$co = new t41_Object_Collection(clone $this->_dataObject);
