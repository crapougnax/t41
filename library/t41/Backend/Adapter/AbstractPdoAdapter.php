	...
	public function create(ObjectModel\DataObject $do)
	{
		...
		foreach ($do->getProperties() as $property) {
			...
			foreach ($collection->getMembers() as $member) {
				...
				$member->getProperty($property->getParameter('keyprop'))->setValue($uri);
				$member->save();
			}
		}
...
//		var_dump($data); die;
		...
		try {
			...
			if (count($data['collections']) == 0) return $ures;
...
/*				foreach ($collection->getValue()->getMembers() as $member) {
...
//			$this->_ressource->commit();
	...
	{	
		...
		try {
			...
			$this->_setLastQuery('delete', $data, array('table' => $table));
...
//		echo $select; die;
		...
		foreach ($result as $key => $val) {
			...
			$result[$key] = implode(Mapper::VALUES_SEPARATOR, $val);
		}
		...
		foreach ($clauses as $key => $clause) {
			...
			if ($value instanceof ObjectModel\BaseObject || $value instanceof ObjectModel\DataObject) {
				...
				if ($value->getUri() == null) {
					...
					throw new Exception(array("OBJECT_HAS_NO_URI", $value->getClass()));
				}
