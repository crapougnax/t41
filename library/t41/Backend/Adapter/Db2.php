 ...
 * @license    http://www.t41.org/license/new-bsd     New BSD License
 */
class t41_Backend_Adapter_Db2 extends t41_Backend_Adapter_Abstract {
	...
	protected function _connect()
	{
		...
		try {
			...
			require_once 't41/Backend/Exception.php';
			throw new t41_Backend_Exception($e->getMessage() . $e->getTraceAsString());
		}		
	}
	...
	public function create(t41_Data_Object $do)
	{
		...
		if (! $table) {
			...
			require_once 't41/Backend/Exception.php';
			throw new t41_Backend_Exception('BACKEND_MISSING_DBTABLE_PARAM');
		}
...
//			Zend_Debug::dump($recordSet);
		...
		try {
			...
			if (true) {
				throw new t41_Backend_Exception("Error Creating Record: " . $e->getMessage);
			} else {
				return false;
			}
		}
	...
	{	
		...
		if (! $table) {
			...
			require_once 't41/Backend/Exception.php';			
			throw new t41_Backend_Exception('MISSING_DBTABLE_PARAM');
		}
	...
	public function update(t41_Data_Object $do)
	{
		...
		foreach ($data as $key => $value) {
			...
			if ($value instanceof t41_Object_Uri) {
	...
	public function find(t41_Object_Collection $collection, $returnCount = false)
	{
		...
		$pkey = $this->_mapper ? $this->_mapper->getPrimaryKey($class) : t41_Backend::DEFAULT_PKEY;
		...
		$select = $this->_ressource->select();
		$select->from($table, $returnCount ? new Zend_Db_Expr("COUNT(*) AS " . t41_Backend::MAX_ROWS_IDENTIFIER) : $pkey);
		...
		if ($returnCount == true) {
			...
			return $result[0][t41_Backend::MAX_ROWS_IDENTIFIER];
		}

		// convert array of primary keys to strings
		foreach ($result as $key => $val) {
			
			$result[$key] = implode(t41_Mapper::VALUES_SEPARATOR, $val);
		}
...
//		Zend_Debug::dump($result); die;
	...
	protected function _buildConditionStatement($field, array $clauses, $mode = 'AND')
	{
		...
		foreach ($clauses as $key => $clause) {
			 ...
			 */
			if ($value instanceof t41_Object_Model || $value instanceof t41_Data_Object) {
			...
			if ($fuzzy) {
				...
				$_operators[t41_Condition::OPERATOR_EQUAL]	= 'LIKE';
				$_operators[t41_Condition::OPERATOR_DIFF]	= 'NOT LIKE';
			}
