...
/** Required files */
require_once 't41/Backend/Adapter/Filesystem.php';
 ...
 * 
 */
class t41_Backend_Adapter_Csv extends t41_Backend_Adapter_Filesystem {
	...
	{
		$url = str_replace('{basepath}', t41_Core::getBasePath(), $this->_uri->getUrl());
		...
		if ($this->_mapper) {
			...
			throw new t41_Backend_Exception("Csv backends need a mapper where datastores are defined");
		}
		...
		try { 
			...
			throw new t41_Backend_Exception("Error opening file: " . $e->getMessage());
		}
	...
	protected function _prepareConditionBlock(t41_Object_Collection $collection)
	{
		...
		foreach ($collection->getConditions() as $condition) {
			...
			foreach ($clauses as $clause) {
				...
				if (! isset($mode)) $mode = isset($clause['mode']) ? $clause['mode'] : t41_Condition::MODE_AND;
				...
				foreach ($ops as $op) {
					...
					switch ($op) {
						...
						case t41_Condition::OPERATOR_BEGINSWITH:
							$prefix = '^'; 
							break;
						
						case t41_Condition::OPERATOR_ENDSWITH:
							$suffix = '$';
							break;
		...
		foreach ($this->_conditionsBlock as $key => $modes) {
			...
			foreach ($modes as $mode => $patterns) {
				...
				foreach ($patterns as $pattern) {
					...
					if ($mode == t41_Condition::MODE_AND && $result == 0) {
						return false;
					}
					
					/* in OR mode, one match is enough */
					if ($mode == t41_Condition::MODE_OR && $result != 0) {

						$status = true;
					}
				}
				
				if ($status == false && $mode == t41_Condition::MODE_OR) {
